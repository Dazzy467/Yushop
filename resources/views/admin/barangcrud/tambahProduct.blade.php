    ...
    <div class="py-12 ml-10 sm:ml-48">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form method="post" action="{{ route('admin.manageProduct.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                    @csrf
                    @method('post')
            
                    <div>
                        <x-input-label for="namaBarang" :value="__('Nama barang')" />
                        <x-text-input id="namaBarang" name="namaBarang" type="text" class="mt-1 block w-full" required autofocus autocomplete="namaBarang" />
                        <x-input-error class="mt-2" :messages="$errors->get('namaBarang')" />
                    </div>
            
                    <div>
                        <x-input-label for="stokBarang" :value="__('Stok')" />
                        <x-text-input id="stokBarang" name="stokBarang" type="number" class="mt-1 block w-full" required autocomplete="stokBarang" />
                        <x-input-error class="mt-2" :messages="$errors->get('stokBarang')" />
                    </div>
            
                    <div>
                        <x-input-label for="hargaBarang" :value="__('Harga')" />
                        <x-text-input id="hargaBarang" name="hargaBarang" type="number" class="mt-1 block w-full" required autofocus autocomplete="hargaBarang" />
                    ...
                    <div>
                        ...
                        <input class="w-[50%] border-[5px] rounded" type="file" name="image_path" required/>
